...
void Midi_Request_Settings(byte note){
  ...
  if(get127 && get0){
    ...
    usbMIDI.sendControlChange(3, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(4, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(5, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(6, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(7, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(8, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(9, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(10, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(11, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(12, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(13, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(14, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(15, midiValueLocal, mySettings.midiChannel);

    // Filter 1 D-F-F
    midiValueLocal =  mySettings.Filter1_Active_Frequenz_Tracking_Target;
    usbMIDI.sendControlChange(16, midiValueLocal, mySettings.midiChannel);   

    // Filter 2 D-F-F
    midiValueLocal =  mySettings.Filter2_Active_Frequenz_Tracking_Target;
    usbMIDI.sendControlChange(17, midiValueLocal, mySettings.midiChannel);


    // Filter LFO cycles
    midiValueLocal =  mySettings.filter_lfo_cycles;
    usbMIDI.sendControlChange(18, midiValueLocal, mySettings.midiChannel);   

    // MOD LFO 2 cycles
    midiValueLocal =  mySettings.mod_lfo_cycles;
    usbMIDI.sendControlChange(19, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(20, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(21, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(22, midiValueLocal, mySettings.midiChannel);

    // 4: filter1 octave CC 23 gui fertig
    midiValueLocal = map(int(mySettings.filter1Octave * 1000.0)+5,0,7000,0,127);
    usbMIDI.sendControlChange(23, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(24, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(25, midiValueLocal, mySettings.midiChannel);  

    // 7: filter2 octave CC 26 gui fertig
    midiValueLocal = map(int(mySettings.filter2Octave * 1000.0)+5,0,7000,0,127);
    usbMIDI.sendControlChange(26, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(28, midiValueLocal, mySettings.midiChannel);
    
    // 10: Filter 1 - 2 crossover mod gui fertig
    midiValueLocal = 0;
    if( mySettings.f1_f2_crossover_Modulation < 0.0) midiValueLocal = 1; 
    usbMIDI.sendControlChange(29, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(30, midiValueLocal, mySettings.midiChannel);
   
    // 12: Guitar Sensing Release gui fertig
    midiValueLocal = map(mySettings.sensingFall+5,0,1000,0,127);
    usbMIDI.sendControlChange(31, midiValueLocal, mySettings.midiChannel);  
    ...
    usbMIDI.sendControlChange(32, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(33, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(34, midiValueLocal, mySettings.midiChannel);

    // 16: OSC1 Volumen gui fertig
    midiValueLocal = map( int(powf(mySettings.osc1_level, 0.2)*1000)+5 ,0 , 1000, 0, 127);
    usbMIDI.sendControlChange(35, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(36, midiValueLocal, mySettings.midiChannel);

    // 18: OSC 1 PulseWide gui fertig
    midiValueLocal =  map( int(mySettings.osc1_pulseWidth*1000)+5 ,10 , 1000, 0, 127);
    usbMIDI.sendControlChange(37, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(41, midiValueLocal, mySettings.midiChannel);

    // OSC 2
     
    // 23: OSC2 Volumen gui fertig
    midiValueLocal = map( int(powf(mySettings.osc2_level, 0.2)*1000)+5 ,0 , 1000, 0, 127);
    usbMIDI.sendControlChange(42, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(43, midiValueLocal, mySettings.midiChannel);

    // 18: OSC 2 PulseWide gui fertig
    midiValueLocal =  map( int(mySettings.osc2_pulseWidth*1000)+5 ,10 , 1000, 0, 127);
    usbMIDI.sendControlChange(44, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(48, midiValueLocal, mySettings.midiChannel);


    // SUB OSC S
    // 16: SUB OSC 1 Volumen gui fertig
    midiValueLocal = map( int(powf(mySettings.sub_osc1_level, 0.2)*1000)+5 ,0 , 1000, 0, 127);
    usbMIDI.sendControlChange(49, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(50, midiValueLocal, mySettings.midiChannel);

    // 18: SUB OSC 1 PulseWide gui fertig
    midiValueLocal =  map( int(mySettings.sub_osc1_pulseWidth*1000)+5 ,10 , 1000, 0, 127);
    usbMIDI.sendControlChange(51, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(55, midiValueLocal, mySettings.midiChannel);

    // SUB OSC 2
     
    // 23: SUB OSC 2 Volumen gui fertig
    midiValueLocal = map( int(powf(mySettings.sub_osc2_level, 0.2)*1000)+5 ,0 , 1000, 0, 127);
    usbMIDI.sendControlChange(56, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(57, midiValueLocal, mySettings.midiChannel);

    // 18: SUB OSC 2 PulseWide gui fertig
    midiValueLocal =  map( int(mySettings.sub_osc2_pulseWidth*1000)+5 ,10 , 1000, 0, 127);
    usbMIDI.sendControlChange(58, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(74, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(97, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(101, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(105, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(106, midiValueLocal, mySettings.midiChannel);   
    ...
    usbMIDI.sendControlChange(107, midiValueLocal, mySettings.midiChannel);
    ...
    usbMIDI.sendControlChange(115, midiValueLocal, mySettings.midiChannel);
